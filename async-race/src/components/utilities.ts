...
function generateName(): string {
  return `${carsNames[generateRandom(carsNames.length)]} ${
    carsModels[generateRandom(carsModels.length)]
  }`;
}
...
export function calcDistance(el1: HTMLElement, el2: HTMLElement): number {
  const res: number =
    el2.getBoundingClientRect().left -
...
async function startCars(
  promises: Array<Promise<IProm>>,
  idents: Array<number>
  ...
  const winnersCar = (storage.cars as Array<ICarWithId>).find(
    (item) => item.id === id
  );
...
export async function race(callback: Callback) {
  const promises = (storage.cars as Array<ICarWithId>).map(({ id }) =>
    callback(id)
  );

  const winners = await startCars(
    promises,
    (storage.cars as Array<ICarWithId>).map((car) => car.id)
  );
